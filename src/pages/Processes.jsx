...
const historic_process_instance_delete_body = (form, historicProcessInstanceIds) => {
...
const Instances = () => {
  const state = useContext(AppState),
    ...
    delete_result = state.api.history.process_instance.delete_async,
    selected = useSignal(new Set()),
  ...
  const submit_delete = async (event) => {
    ...
    const body = historic_process_instance_delete_body(
      event.currentTarget,
      selected_ids,
    );
    await engine_rest.history.process_instance.delete_async(state, body);
    selected.value = new Set();
    fetch_page(0);
  };
  ...
  return !params?.selection_id ? (
    <>
        ...
        <div class="toolbar instance-toolbar">
          <div>
            <button
              ...
              onClick={open_delete}
            >
              {t("processes.instance.historic-delete.open")}
            </button>
            {has_selection ? (
              <small>
                {t("processes.instance.historic-delete.count", {
      ...
      {delete_open.value ? (
        <form class="historic-instance-delete" onSubmit={submit_delete}>
          <fieldset>
            <legend>{t("processes.instance.historic-delete.title")}</legend>
            <label>
              {t("processes.instance.historic-delete.reason")}
              <input type="text" name="deleteReason" />
            </label>
            <div class="button-group">
              <button type="submit" class="danger" disabled={delete_loading}>
                {t("processes.instance.historic-delete.confirm")}
              </button>
          ...
          <RequestState
            ...
            on_success={() => {
              ...
              return batch?.id ? (
                <p class="info-box">
                  {t("processes.instance.historic-delete.batch-created")}{" "}
                  <a href={`/batches/${batch.id}`}>
                    {t("processes.instance.historic-delete.view-batch")}
                  </a>
                </p>
              ) : (
                <p class="info-box">
            ...
            <tr>
              {history_mode ? (
                <th>
                  <input
                    type="checkbox"
                    aria-label={t("processes.instance.historic-delete.select-all")}
                    checked={all_selected}
